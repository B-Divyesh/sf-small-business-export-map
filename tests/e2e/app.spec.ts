...
test('routes return real statuses, complete metadata including Twitter image, valid links, and restore h1 focus', async ({ page, request }) => {
  ...
  const socialImage='https://small-business-export-map.sociobot.in/assets/export-map-og.webp';
  ...
  for(const [path,title,canonical] of routes){const response=await page.goto(path);expect(response?.status()).toBe(200);await expect(page).toHaveTitle(title);await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href',canonical);await expect(page.locator('meta[property="og:url"]')).toHaveAttribute('content',canonical);await expect(page.locator('meta[name="twitter:title"]')).toHaveAttribute('content',title);await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute('content',socialImage);await expect(page.locator('h1')).toBeFocused();expect(await page.locator('header .brand-mark').count()).toBe(1);await expect(page.locator('footer')).toContainText('build polish-5');}
  ...
  const missing=await page.goto('/missing-review-route');expect(missing?.status()).toBe(404);await expect(page).toHaveTitle('Page not found — Export Map');await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute('content',socialImage);await expect(page.getByRole('heading',{name:'This page does not exist.'})).toBeFocused();await expect(page.getByRole('link',{name:'Prepare a CSV'})).toHaveAttribute('href','/#workspace');
